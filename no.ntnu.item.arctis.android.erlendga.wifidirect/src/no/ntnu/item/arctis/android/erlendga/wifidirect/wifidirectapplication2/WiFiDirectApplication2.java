...
public class WiFiDirectApplication2 extends Block {
	...
	public void resetData(int numberOfTimes) {
		if (numberOfTimes == 1) {
		...
		else createAlertDialog("Severe! Channel is probably lost premanently. Try Disable/Re-Enable P2P.");
	}
		...
		activity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (device.status == WifiP2pDevice.UNAVAILABLE) {
					...
					deviceListFragment.updateThisDevice(device);
					if (cameraOn) {
						cameraView.setVisibility(View.GONE);
					}
				}
	...
	public void getFailure(int failureCode) {
		switch (failureCode) {
			...
			case WifiP2pManager.BUSY:
				...
				break;
			case WifiDirect1.NOT_CONNECTED:
				...
				break;
			case WifiDirect1.WIFI_P2P_DISABLED:
				enableProgressBar(false);
